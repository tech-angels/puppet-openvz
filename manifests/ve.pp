  ...
  case $presence {
    "present": {
      ...
      exec {"vzctl set ${veid} --applyconfig ${config} --save":
        unless  => "sh -c '. ${configfile}; test \"\$ORIGIN_SAMPLE\" = \"${config}\"'",
      ...
      case $state {
        "running": {
          ...
          exec {"vzctl set ${veid} --onboot yes --save":
            unless  =>  "sh -c '. ${configfile}; test \"\$ONBOOT\" = \"yes\"'",
            require => Exec["vzctl create ${veid}"],
          }
        }
        ...
        "stopped": {
          ...
          exec {"vzctl set ${veid} --onboot no --save":
            unless  =>  "sh -c '. ${configfile}; test \"\$ONBOOT\" = \"no\"'",
            require => Exec["vzctl create ${veid}"],
          }
        }
      }
      ...
      if $fix_resolve {
        ...
        exec {"resolvconf fix ${veid}":
          command => "sh -c '. ${openvz_conf_file}; cp -f /etc/resolv.conf \$VE_PRIVATE/${veid}/etc/resolv.conf'",
          refreshonly => true,
        }
